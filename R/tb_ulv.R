...
tb_ulv <- function(x, jur = NULL, mun, coldfog){
    ...
    if(TRUE == mun){
        xa <- x %>%
            ...
            dplyr::summarise("Ha Trabajadas hasta la Semana" = sum(Hectareas.trabajadas, na.rm = TRUE),
        ...
        xb <- x %>%
            ...
            dplyr::group_by(Municipio) %>%
            dplyr::summarise("Ha Trabajadas en los dos últimos meses" = sum(Hectareas.trabajadas, na.rm = TRUE),
        ...
        xc <- x %>%
            ...
            dplyr::group_by(Municipio) %>%
            dplyr::summarise("Ha Trabajadas en la semana" = sum(Hectareas.trabajadas, na.rm = TRUE),
        ...
        xabc <- dplyr::left_join(x = dplyr::left_join(x = xa, y = xb, by = "Municipio"),
                                 y = xc,
                                 by = "Municipio") %>%
            tidyr::replace_na(list(`Ha Trabajadas hasta la Semana` = 0,
                                   `Insecticida Consumido hasta la Semana` = 0,
                                   `Ha Trabajadas en los dos últimos meses` = 0,
                                   `Insecticida Consumido en los dos últimos meses` = 0,
                                   `Ha Trabajadas en la semana` = 0,
                                   `Insecticida Consumido en la semana` = 0)) %>%
            dplyr::arrange(desc(`Ha Trabajadas hasta la Semana`))
        ...
        formattable::formattable(xabc,
                    ...
                    list(
                        Municipio = formattable::formatter("span", style = x ~ formattable::style(color =  "black",
                                                                        font.weight = "bold")),
                        "Ha Trabajadas hasta la Semana" = formattable::color_tile("#DeF7E9", "#71CA97"),
                        "Insecticida Consumido hasta la Semana" = formattable::color_tile("white", "orange"),
                        "Ha Trabajadas en los dos últimos meses" = formattable::color_tile("#DeF7E9", "#71CA97"),
                        "Insecticida Consumido en los dos últimos meses" = formattable::color_tile("white", "orange"),
                        "Ha Trabajadas en la semana" = formattable::color_tile("#DeF7E9", "#71CA97"),
